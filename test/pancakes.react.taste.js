...
var pancakesReactTaste = _.extend({}, taste);
...
/**
 * eval code with a mock react object
 * @param code
 * @param isRaw
 */
pancakesReactTaste.validateCode = function validateCode(code, isRaw) {
    var success = isRaw ?
 ...
 * @returns {*}
 */
pancakesReactTaste.getTemplate = function getTemplate(name) {
    var filePath = __dirname + '/../lib/transformers/react.' + name + '.template';
    var file = fs.readFileSync(path.normalize(filePath));
    return dot.template(file);
};

// expose the pancakes react taste object
module.exports = pancakesReactTaste;